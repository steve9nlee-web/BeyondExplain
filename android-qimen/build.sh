#!/usr/bin/env bash
# Builds qimen-sideload.apk from the web app using only open-source tools:
...
#
# Usage: ./build.sh            (from the android-qimen/ directory)
# Output: ../dist/qimen-sideload.apk
set -euo pipefail
cd "$(dirname "$0")"

ANDROID_JAR="${ANDROID_JAR:-build/android-stub.jar}"
DX="$(command -v dalvik-exchange || command -v dx)"
KEYSTORE="${KEYSTORE:-build/qimen.keystore}"
KS_PASS="${KS_PASS:-qimen-dev}"
...
# Bundle the web app into assets/www (only what the chart page needs).
mkdir -p assets/www/css assets/www/js/vendor
cp ../qimen.html assets/www/
cp ../css/qimen.css assets/www/css/
cp ../js/qimen-engine.js ../js/qimen-data.js ../js/qimen-app.js assets/www/js/
cp ../js/vendor/lunar.js ../js/vendor/LICENSE-lunar-javascript assets/www/js/vendor/
...
javac --release 8 -classpath "$ANDROID_JAR" -d build/obj \
  build/gen/com/beyondexplain/qimen/R.java \
  src/com/beyondexplain/qimen/MainActivity.java
...
if [ ! -f "$KEYSTORE" ]; then
  keytool -genkeypair -keystore "$KEYSTORE" -storepass "$KS_PASS" \
    -keypass "$KS_PASS" -alias qimen -keyalg RSA -keysize 2048 \
    -validity 10000 -dname "CN=Qi Men Hourly Chart"
fi
apksigner sign --ks "$KEYSTORE" --ks-pass "pass:$KS_PASS" \
  --key-pass "pass:$KS_PASS" --out ../dist/qimen-sideload.apk build/app.aligned.apk

apksigner verify --print-certs ../dist/qimen-sideload.apk | head -3
echo "Built ../dist/qimen-sideload.apk"
